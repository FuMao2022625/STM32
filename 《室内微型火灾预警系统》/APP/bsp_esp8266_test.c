...
{
	ESP8266_Cmd("AT+CIPCLOSE=5", "OK", NULL, 1000);  // 关闭所有连接
	ESP8266_Cmd("AT+CWQAP", "OK", NULL, 2000);       // 断开当前 AP（如果需要）
	Delayms(1000);
    printf( "\r\n正在配置 ESP8266 ......\r\n" );
    printf( "\r\n使能 ESP8266 ......\r\n" );
...
/**
  * @brief  透传模式下发送字符串到服务器
  * @param  data 待发送的字符串（以 '\0' 结尾）
  * @retval 无
  */
void ESP8266_TransparentSend(char *data)
{
    macESP8266_Usart("%s", data);
}

void ESP8266_ProcessRecvData (void)
{
    uint8_t i;
    /* 如果接收到 ESP8266 的数据（服务器下发） */
    if (strEsp8266_Fram_Record.InfBit.FramFinishFlag)
    {
        // 将接收到的数据通过串口1打印（调试）
        for (i = 0; i < strEsp8266_Fram_Record.InfBit.FramLength; i++)
        ...
        // 在这里添加你的数据解析代码
        // 例如：if (strstr(strEsp8266_Fram_Record.Data_RX_BUF, "led_on")) { ... }
